...
pub fn load_data_block_local(
    ...
    segment.id = decode_utils::decode_id(bytes, stream_pos);
    ...
    for i in 0..segment.segments.len() {
        ...
        match data_type {
            ...
            x if x == DataTypeIdFormat::ArrayOfVectorOf4Single as i32 => {
                let res = decode_utils::decode_single_vector4_array(bytes, stream_pos);
            ...
            x if x == DataTypeIdFormat::ArrayOfVectorOf8Single as i32 => {
                if format_version == 1 {
                    let res = decode_utils::decode_single_vector7_array(bytes, stream_pos);
                    segment.segments[i].value = serialize::single_vector_7_vec_to_string_vec(res);
                } else {
                    let res = decode_utils::decode_single_vector8_array(bytes, stream_pos);
            ...
            x if x == DataTypeIdFormat::EncodedString as i32 => {
                let res = decode_utils::decode_u64_string(bytes, stream_pos);

                segment.segments[i].value = vec![res];
                continue;
            }
            x if x == DataTypeIdFormat::IdType3 as i32 => {
                let res = decode_utils::decode_id(bytes, stream_pos);
            ...
            x if x == DataTypeIdFormat::Int32 as i32 => {
                let res = decode_utils::decode_int32(bytes, stream_pos);

                segment.segments[i].value = vec![res.to_string()];
                continue;
            }
            x if x == DataTypeIdFormat::Int64 as i32 => {
                let res = decode_utils::decode_int64(bytes, stream_pos);

                segment.segments[i].value = vec![res.to_string()];
                continue;
            }
            x if x == DataTypeIdFormat::UInt32Type2 as i32 => {
                let res = decode_utils::decode_u32(bytes, stream_pos);
            ...
            x if x == DataTypeIdFormat::VectorOf8Single as i32 => {
                if format_version == 1 {
                    let res = decode_utils::decode_single_vector7(bytes, stream_pos);

                    segment.segments[i].value = vec![serialize::single_vector_7_to_string(res)];
                } else {
                    let res = decode_utils::decode_single_vector8(bytes, stream_pos);
            ...
            x if x == DataTypeIdFormat::ArrayOfVectorOf2Int32 as i32 => {
                let res = decode_utils::decode_int32_vector2(bytes, stream_pos);
